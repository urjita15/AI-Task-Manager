...
import joblib
import os
import json # Ensure json is imported for correct metrics loading
...
def load_artifacts():
    ...
    try:
        ...
        # FIX: Load metrics as a Python dictionary using 'json'
...
# FIX: Ensure Streamlit config starts cleanly without extra text in the main body
...
# Page: Prioritize & Manage Tasks
    ...
    if df.empty:
        ...
        # FIX: Corrected function name to use pd.DataFrame([task_row]).T for display
...
# Page: Insights / Models
    ...
    # FIX: Use vstack to combine single-row feature vectors into a single test set matrix (Dimensionality fix)
